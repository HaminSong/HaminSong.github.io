...
### 1. 핵심 규칙
게임 시작 시 Fade In이 되고 바위의 모습이 보임
플레이어는 바위를 조종하여 성벽 및 각종 구조물들을 부숴 포인트를 획득하거나 구조물들을 피해 성벽에 도달하여 
성벽에 피해를 주면 한판이 끝나게 된다. 또한 여기서 획득한 스톤포인트를 이용하여 새로운 바위를 만들고 해당 
바위를 이용하여 다시 새로운 판을 시작한다. 성벽을 파괴하거나 스톤포인트로 새로운 바위를 생성할 수 없게 되면 게임이 종료되고 해당 스테이지의 클리어 여부를 
판별하게 된다. 스테이지를 클리어하면 다음 스테이지로 넘어갈 수 있게 된다.
...
### 1) 플레이어, 구조물 충돌 공식
1-1) 플레이어와 구조물이 부딪히면 플레이어의 magnitude를 계산한다.

1-2) 플레이어의 충돌력이 구조물의 체력을 넘는다면 구조물을 파괴한다.

### 2) 플레이어, 성벽 충돌 공식
2-1) 플레이어와 성벽이 부딪히면 플레이어의 magnitude를 계산한다.

2-2) 플레이어의 충돌력만큼 성벽의 체력에 데미지를 준다.
...
### 적 유닛 및 구조물
- 적의 구조물 파괴 시 재화획득(재화가 UI창으로 자동적으로 들어가는 것처럼 보이도록하기)
